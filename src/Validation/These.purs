module Complex.Validation.These where
...
data Erroring e a
  = Success a
  | Error (NEA.NonEmptyArray (NEA.NonEmptyArray e)) (Maybe a)
...
instance bifunctorErroring :: Bifunctor Erroring where
  ...
  bimap f g (Error e ma) = Error (map (map f) e) (map g ma)
...
altThese (Success a) (Error _ (Just b)) = Success (Both a b)
altThese (Success a) (Error _ Nothing) = Success (This a)
altThese (Error _ Nothing) (Success b) = Success (That b)
altThese (Error _ (Just a)) (Success b) = Success (Both a b)
altThese (Error e1s ma) (Error e2s mb) = Error (e1s <|> e2s) case ma, mb of
...
escalate (W.WriterT (Error es Nothing)) = Error es Nothing
escalate (W.WriterT (Error es (Just (Tuple a ws)))) =
  case NEA.fromArray ws of
    Nothing -> Error es (Just a)
    Just es' -> Error (es <> (pure <$> es')) (Just a)
escalate (W.WriterT (Success (Tuple a ws))) =
  case NEA.fromArray ws of
    Just es -> Error (pure <$> es) (Just a)
    Nothing -> Success a
